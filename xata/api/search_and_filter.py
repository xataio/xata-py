...
class Search_and_filter(Namespace):
    ...
    def query(self, table_name: str, payload: dict, db_name: str = None, branch_name: str = None) -> Response:
       ...
       guide/get).  ### Column selection  If the `columns` array is not specified, all columns
       are included.  For link fields, only the ID column of the linked records is included in
       the response.  If the `columns` array is specified, only the selected and internal columns
       `id` and `xata` are included.  The `*` wildcard can be used to select all columns.  For
       objects and link fields, if the column name of the object is specified, we include all of
       its sub-keys.  If only some sub-keys are specified (via dotted notation, e.g.
       `"settings.plan"` ), then only those sub-keys from the object are included.  By the way of
       example, assuming two tables like this:  ```json {"truncate": true} {   "tables": [     {
       "name": "teams",       "columns": [         {           "name": "name",           "type":
       "string"         },         {           "name": "owner",           "type": "link",
       "link": {             "table": "users"           }         },         {           "name":
       "foundedDate",           "type": "datetime"         },       ]     },     {       "name":
       "users",       "columns": [         {           "name": "email",           "type": "email"
       ...
       "table": "teams"           }         }       ]     }   ] } ```  A query like this:
       ```json POST /db/<dbname>:<branch>/tables/<table>/query {   "columns": [     "name",
       "address.*"   ] } ```  returns objects like:  ```json {   "name": "Kilian",   "address": {
       "street": "New street",     "number": 41,     "zipcode": 10407   } } ```  while a query
       like this:  ```json POST /db/<dbname>:<branch>/tables/<table>/query {   "columns": [
       "name",     "address.street"   ] } ```  returns objects like:  ```json {   "id": "id1"
       "xata": {     "version": 0   }   "name": "Kilian",   "address": {     "street": "New
       street"   } } ```  If you want to return all columns from the main table and selected
       columns from the linked table, you can do it like this:  ```json {   "columns": ["*",
       "team.name"] } ```  The `"*"` in the above means all columns, including columns of
       objects.  This returns data like:  ```json {   "id": "id1"   "xata": {     "version": 0
       }   "name": "Kilian",   "email": "kilian@gmail.com",   "address": {     "street": "New
       street",     "number": 41,     "zipcode": 10407   },   "team": {     "id": "XX",
       "xata": {       "version": 0     },     "name": "first team"   } } ```  If you want all
       columns of the linked table, you can do:  ```json {   "columns": ["*", "team.*"] } ```
       This returns, for example:  ```json {   "id": "id1"   "xata": {     "version": 0   }
       "name": "Kilian",   "email": "kilian@gmail.com",   "address": {     "street": "New
       street",     "number": 41,     "zipcode": 10407   },   "team": {     "id": "XX",
       "xata": {       "version": 0     },     "name": "first team",     "code": "A1",
       "foundedDate": "2020-03-04T10:43:54.32Z"   } } ```  ### Filtering  There are two types of
       operators:  - Operators that work on a single column: `$is`, `$contains`, `$pattern`,
       `$includes`, `$gt`, etc.  - Control operators that combine multiple conditions: `$any`,
       `$all`, `$not` ,   `$none`, etc.  All operators start with an `$` to differentiate them
       from column names (which are not allowed to start with a dollar sign).  #### Exact
       matching and control operators  Filter by one column:  ```json {   "filter": {
       "<column_name>": "value"   } } ```  This is equivalent to using the `$is` operator:
       ```json {   "filter": {     "<column_name>": {       "$is": "value"     }   } } ```  For
       example:  ```json {   "filter": {     "name": "r2"   } } ```  Or:  ```json {   "filter": {
       "name": {       "$is": "r2"     }   } } ```  For objects, both dots and nested versions
       work:  ```json {   "filter": {     "settings.plan": "free"   } } ```  ```json {
       "filter": {     "settings": {       "plan": "free"     }   } } ```  If you want to OR
       together multiple values, you can use the `$any` operator with an array of values:
       ```json {   "filter": {     "settings.plan": { "$any": ["free", "paid"] }   } } ```  If
       you specify multiple columns in the same filter, they are logically AND'ed together:
       ```json {   "filter": {     "settings.dark": true,     "settings.plan": "free"   } } ```
       The above matches if both conditions are met.  To be more explicit about it, you can use
       `$all` or `$any`:  ```json {   "filter": {     "$any": {       "settings.dark": true,
       "settings.plan": "free"     }   } } ```  The `$all` and `$any` operators can also receive
       an array of objects, which allows for repeating column names:  ```json {   "filter": {
       "$any": [       {         "name": "r1"       },       {         "name": "r2"       }     ]
       } } ```  You can check for a value being not-null with `$exists`:  ```json {   "filter": {
       "$exists": "settings"   } } ```  This can be combined with `$all` or `$any` :  ```json {
       "filter": {     "$all": [       {         "$exists": "settings"       },       {
       "$exists": "name"       }     ]   } } ```  Or you can use the inverse operator
       `$notExists`:  ```json {   "filter": {     "$notExists": "settings"   } } ```  ####
       Partial match  `$contains` is the simplest operator for partial matching.  Note that
       `$contains` operator can cause performance issues at scale, because indices cannot be
       used.  ```json {   "filter": {     "<column_name>": {       "$contains": "value"     }   }
       } ```  Wildcards are supported via the `$pattern` operator:  ```json {   "filter": {
       "<column_name>": {       "$pattern": "v*alu?"     }   } } ```  The `$pattern` operator
       accepts two wildcard characters: * `*` matches zero or more characters * `?` matches
       exactly one character  If you want to match a string that contains a wildcard character,
       you can escape them using a backslash (`\`). You can escape a backslash by usign another
       backslash.  You can also use the `$endsWith` and `$startsWith` operators:  ```json {
       "filter": {     "<column_name>": {       "$endsWith": ".gz"     },     "<column_name>": {
       "$startsWith": "tmp-"     }   } } ```  #### Numeric or datetime ranges  ```json {
       "filter": {     "<column_name>": {       "$ge": 0,       "$lt": 100     }   } } ``` Date
       ranges support the same operators, with the date using the format defined in [RFC
       3339](https://www.rfc-editor.org/rfc/rfc3339): ```json {   "filter": {
       "<column_name>": {       "$gt": "2019-10-12T07:20:50.52Z",       "$lt":
       "2021-10-12T07:20:50.52Z"     }   } } ``` The supported operators are `$gt`, `$lt`, `$ge`,
       `$le`.  #### Negations  A general `$not` operator can inverse any operation.  ```json {
       "filter": {     "$not": {       "<column_name1>": "value1",       "<column_name2>":
       "value1"     }   } } ```  Note: in the above the two condition are AND together, so this
       does (NOT ( ... AND ...))  Or more complex:  ```json {   "filter": {     "$not": {
       "$any": [         {           "<column_name1>": "value1"         },         {
       "$all": [             {               "<column_name2>": "value2"             },
       {               "<column_name3>": "value3"             }           ]         }       ]
       }   } } ```  The `$not: { $any: {}}` can be shorted using the `$none` operator:  ```json {
       "filter": {     "$none": {       "<column_name1>": "value1",       "<column_name2>":
       "value1"     }   } } ```  In addition, you can use operators like `$isNot` or `$notExists`
       to simplify expressions:  ```json {   "filter": {     "<column_name>": {       "$isNot":
       "2019-10-12T07:20:50.52Z"     }   } } ```  #### Working with arrays  To test that an array
       contains a value, use `$includesAny`.  ```json {   "filter": {     "<array_name>": {
       "$includesAny": "value"     }   } } ```  ##### `includesAny`  The `$includesAny` operator
       accepts a custom predicate that will check if any value in the array column matches the
       predicate.  The `$includes` operator is a synonym for the `$includesAny` operator.  For
       example a complex predicate can include the `$all` , `$contains` and `$endsWith`
       operators:  ```json {   "filter": {     "<array name>": {       "$includes": {
       "$all": [           { "$contains": "label" },           { "$not": { "$endsWith": "-debug"
       } }         ]       }     }   } } ```  ##### `includesNone`  The `$includesNone` operator
       succeeds if no array item matches the predicate.  ```json {   "filter": {
       "settings.labels": {       "$includesNone": [{ "$contains": "label" }]     }   } } ``` The
       above matches if none of the array values contain the string "label".  ##### `includesAll`
       The `$includesAll` operator succeeds if all array items match the predicate.  Here is an
       ...
       the current cursor - `before`: Return the previous page 'before' the current cursor.  -
       `start`: Resets the given cursor position to the beginning of the query result set.  Will
       return the first N records from the query result, where N is the `page.size` parameter.  -
       `end`: Resets the give cursor position to the end for the query result set.  Returns the
       last N records from the query result, where N is the `page.size` parameter.  The request
       will fail if an invalid cursor value is given to `page.before`, `page.after`, `page.start`
       , or `page.end`. No other cursor setting can be used if `page.start` or `page.end` is set
       in a query.  If both `page.before` and `page.after` parameters are present we treat the
       request as a range query.  The range query will return all entries after `page.after`, but
       before `page.before`, up to `page.size` or the maximum page size.  This query requires
    ...
    def ask(self, table_name: str, payload: dict, db_name: str = None, branch_name: str = None, response_content_type: str = "application/json") -> Response:
       """
    ...
    def summarize(self, table_name: str, payload: dict, db_name: str = None, branch_name: str = None) -> Response:
       ...
       questions you can ask of your data:**  How many records do I have in this table?  - Set
       `columns: []` as we we want data from the entire table, so we ask for no groups.  - Set
       `summaries: {"total": {"count": "*"}}` in order to see the count  of all records.  We use
       `count: *` here we'd like to know the total  amount of rows; ignoring whether they are
       `null` or not.  What are the top total sales for each product in July 2022 and sold  more
       than 10 units?  - Set `filter: {soldAt: {   "$ge": "2022-07-01T00:00:00.000Z",    "$lt":
       "2022-08-01T00:00:00.000Z"} }`  in order to limit the result set to sales recorded in July
       2022. - Set `columns: [product_name]` as we'd like to run calculations on  each unique
       product name in our table.  Setting `columns` like this will  produce one row per unique
       product name.  - Set `summaries: {"total_sales": {"count": "product_name"}}` as we'd  like
       to create a field called "total_sales" for each group.  This field  will count all rows in
       each group with non-null product names.  - Set `sort: [{"total_sales": "desc"}]` in order
       to bring the rows with  the highest total_sales field to the top.  - Set `summariesFilter:
       {"total_sales": {"$ge": 10}}` to only send back data  with greater than or equal to 10
       units.  `columns`: tells Xata how to create each group.  If you add `product_id`  we will
       create a new group for every unique `product_id`.  `summaries`: tells Xata which
       calculations to run on each group.  Xata currently supports count, min, max, sum, average.
       `sort`: tells Xata in which order you'd like to see results.  You may  sort by fields
       specified in `columns` as well as the summary names  defined in `summaries`.  note:
       Sorting on summarized values can be slower on very large tables;  this will impact your
       rate limit significantly more than other queries.  Try use `filter` to reduce the amount
