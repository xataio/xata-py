...
class BulkProcessor(object):
    ...
    def process(self, id: int):
        ...
        Process the records
        """
        self.logger.debug("thread #%d: starting bulk processor [thread_pool_size=%d, batch_size=%d, flush_interval=%d, processing_timeout=%s" % (id, len(self.thread_workers), self.batch_size, self.flush_interval, self.processing_timeout))
        while True:
            ...
            if 'table' in batch and len(batch['records']) > 0:
                ...
                if r.status_code != 200:
                    self.logger.error("thread #%d: unable to process batch for table '%s', with error: %d - %s" % (id, batch['table'], r.status_code, r.json()['message']))
        ...
        def next_batch(self) -> dict:
            ...
            with self.store[table_name]['lock']:
                ...
                if flush_needed and len(self.store[table_name]['records']) > 0:
                    self.logger.debug("flushing table '%s' with %d records after interval %s > %d" % (table_name, len(self.store[table_name]['records']), time_elapsed, self.flush_interval))
                # pop records ?
