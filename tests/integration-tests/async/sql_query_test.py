...
class TestSqlquery_async(object):
    ...
    def test_query_async(self):
        r = self.client.sql().query_async('SELECT * FROM "Users" LIMIT 5')
        assert r.is_success()
        assert "records" in r
        assert len(r["records"]) == 5

    def test_query_on_non_existing_table(self):
        r = self.client.sql().query_async('SELECT * FROM "DudeWhereIsMyTable"')
        assert not r.is_success()

    def test_query_with_invalid_statement(self):
        r = self.client.sql().query_async("SELECT ' fr_o-")
        assert not r.is_success()

    def test_query_statement_with_missing_params(self):
        r = self.client.sql().query_async("SELECT * FROM \"Users\" WHERE email = '$1'")
        assert r.is_success()
        assert len(r) == 0

    def test_query_statement_with_params_and_no_param_references(self):
        r = self.client.sql().query_async('SELECT * FROM "Users"', ["This is important"])
        assert not r.is_success()

    def test_query_statement_with_too_many_params(self):
        r = self.client.sql().query_async(
    ...
    def test_query_statement_with_not_enough_params(self):
        r = self.client.sql().query_async('INSERT INTO "Users" (name, email) VALUES ($1, $2)', ["Shrek"])
        assert not r.is_success()
    ...
    def test_query_with_params(self):
        r = self.client.sql().query_async('SELECT * FROM "Users" WHERE email = $1', ["keanu@example.com"])
        assert r.is_success()
        assert "records" in r
