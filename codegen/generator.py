...
def generate_namespace(namespace: dict, scope: str, spec_version: str, spec_base_url: str):
    ...
    vars = {
        ...
        "class_description": namespace["description"] if "description" in namespace else namespace["x-displayName"],
        "spec_scope": scope,
        "spec_version": spec_version,
        "spec_base_url": spec_base_url,
    }
    out = Template(filename='codegen/namespace.tpl.py', output_encoding='utf-8').render(**vars)
...
def generate_endpoint(path: str, method: str, endpoint: dict, parameters: list, references: dict) -> str:
    ...
    vars = {
        ...
        "description": endpoint["description"] if "description" in endpoint else endpoint["summary"],
...
#        "responses": list(endpoint["responses"].keys()),
    ...
    out = Template(filename='codegen/endpoint.tpl.py', output_encoding='utf-8').render(**vars)
    return out
...
if __name__ == "__main__":
    ...
    logging.info("filtering %d namespaces to ensure endpoints exist .." % len(spec['tags']))
